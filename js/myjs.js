...
function checkform(obj,chk){
    var f = 0;
    jQuery.each(obj, function(index, val) {
      if($(this).val()=='' && f == 0){
        show_toastr('toast-top-full-width','error','請輸入'+$(this).attr('alt'),'');
        f = 1;
      }

      if($(this).attr('id')=='email' && !ValidEmail($(this).val())&&f==0){
            show_toastr('toast-top-full-width','error','請輸入正確EMAIL格式！','');
            f = 1;
        }

        if($(this).attr('id')=='tel' && !ValidTel($(this).val())&&f==0){
            show_toastr('toast-top-full-width','error','請輸入純數字的電話號碼！','');
            f = 1;
        }
    });

    if(f==0 && !chk.prop('checked')){
        show_toastr('toast-top-full-width','error','您尚未閱讀完活動說明，並同意參加活動！','');
        return;
    }

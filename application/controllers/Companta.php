...
class Companta extends CI_Controller {
	...
	function __construct() {
		...
		if(!in_array($this->router->method,$ar) && $bbb != '1'){
			header('Location: '.site_url('Companta/ask'));
		}
	...
	public function setask() {
		$_SESSION['have_ask'] = '1';
		redirect('/Companta/index', 'refresh');
	}
...
// 				print_r($e);
	...
	public function redirect($fbid=0){
		if($fbid==0):
			...
			echo '<script>window.open("'.APP_HOST.'index.php/Companta/message/'.$fbid.'","_top");</script>';
		endif;
		exit;
	}
...
// 		$this->load->library('bitly');
		$url = site_url('Companta/redirect').'/'.$fbid;
// 		$bitly = '';
// 		$bitly = $this->bitly->shorten($url);
// 		$bitly = $bitly->$url->shortUrl;
